...
const (
	BMCSettingFinalizer = "baseboard.metal.ironcore.dev/bmcsettings"
...
func (r *BMCSettingsReconciler) reconcileExists(ctx context.Context, settings *baseboardv1alpha1.BMCSettings) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsReconciler) shouldDelete(ctx context.Context, bmcSetting *baseboardv1alpha1.BMCSettings) (bool, error) {
	isProgressing := func() (bool, error) {
		if bmcSetting.Status.State != baseboardv1alpha1.BMCSettingsStateInProgress {
			return false, nil
		}
		...
		return utils.IsAnyServerMaintenanceActive(ctx, r.Client, refs)
	}
	return utils.ShouldProceedWithDeletion(ctx, bmcSetting, BMCSettingFinalizer, isProgressing)
}

func (r *BMCSettingsReconciler) delete(ctx context.Context, settings *baseboardv1alpha1.BMCSettings) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsReconciler) cleanupServerMaintenanceReferences(ctx context.Context, settings *baseboardv1alpha1.BMCSettings) error {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsReconciler) reconcile(ctx context.Context, settings *baseboardv1alpha1.BMCSettings) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsReconciler) ensureBMCSettingsMaintenanceStateTransition(ctx context.Context, settings *baseboardv1alpha1.BMCSettings, bmcObj *metalv1alpha1.BMC) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
	...
	case "", baseboardv1alpha1.BMCSettingsStatePending:
		...
		if bmcObj.Status.FirmwareVersion != settings.Spec.Version {
			...
			state = baseboardv1alpha1.BMCSettingsStatePending
...
func (r *BMCSettingsReconciler) handleSettingInProgressState(ctx context.Context, settings *baseboardv1alpha1.BMCSettings, bmcObj *metalv1alpha1.BMC, bmcClient bmc.BMC) (ctrl.Result, error) {
	...
	if len(settingsDiff) == 0 {
		return ctrl.Result{}, r.updateBMCSettingsStatus(ctx, settings, baseboardv1alpha1.BMCSettingsStateApplied, nil)
	}
...
func (r *BMCSettingsReconciler) updateSettingsAndVerify(ctx context.Context, settings *baseboardv1alpha1.BMCSettings, bmcObj *metalv1alpha1.BMC, settingsDiff schemas.SettingsAttributes, bmcClient bmc.BMC) (ctrl.Result, error) {
	...
	changesIssued, err := utils.GetCondition(r.Conditions, settings.Status.Conditions, ConditionBMCSettingsChangesIssued)
	if err != nil {
		...
		if len(pendingAttr) == 0 {
			...
			if err != nil {
				...
				if errors.As(err, &invalidSettingsErr) {
					...
					err := r.updateBMCSettingsStatus(ctx, settings, baseboardv1alpha1.BMCSettingsStateFailed, inValidSettings)
					return ctrl.Result{}, err
				}
			...
			log.V(1).Info("BMC settings issued successfully", "SettingKeys", utils.SettingKeys(settingsDiff))
	...
	BMCSettingsVerifiedCondition, err := utils.GetCondition(r.Conditions, settings.Status.Conditions, ConditionBMCSettingsChangesVerified)
	if err != nil {
	...
	if len(settingsDiff) == 0 {
		...
		return ctrl.Result{}, r.updateBMCSettingsStatus(ctx, settings, baseboardv1alpha1.BMCSettingsStateApplied, BMCSettingsVerifiedCondition)
	}
...
func (r *BMCSettingsReconciler) persistApplyCycleConditions(ctx context.Context, settings *baseboardv1alpha1.BMCSettings, changesIssued *metav1.Condition, resetBMCReq bool, applyResults map[string]bmc.ApplyResult) error {
	...
	verifiedCond, err := utils.GetCondition(r.Conditions, settings.Status.Conditions, ConditionBMCSettingsChangesVerified)
	if err != nil {
	...
	if resetBMCReq {
		if err := r.Conditions.Update(
			resetCond,
			conditionutils.UpdateStatus(corev1.ConditionFalse),
			conditionutils.UpdateReason(constants.ReasonResetRequired),
		...
		if err := r.Conditions.Update(
			resetCond,
			conditionutils.UpdateStatus(corev1.ConditionTrue),
			conditionutils.UpdateReason(constants.ReasonNoResetRequired),
...
func (r *BMCSettingsReconciler) handleBMCPowerState(
	...
	bmcSetting *baseboardv1alpha1.BMCSettings,
) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
	...
	case metalv1alpha1.UnknownPowerState:
		BMCPoweredOffCond, err := utils.GetCondition(r.Conditions, bmcSetting.Status.Conditions, ConditionBMCPoweredOff)
		if err != nil {
	...
	default:
		...
		err = r.updateBMCSettingsStatus(ctx, bmcSetting, baseboardv1alpha1.BMCSettingsStateFailed, BMCPoweredOffCond)
		return true, err
	}
	return false, nil
}
...
func (r *BMCSettingsReconciler) handleBMCReset(
	...
	log := ctrl.LoggerFrom(ctx)
	resetBMC, err := utils.GetCondition(r.Conditions, settings.Status.Conditions, conditionType)
	if err != nil {
	...
	if conditionType == ConditionBMCResetPostSettingApply && resetBMC.Status == metav1.ConditionTrue && resetBMC.Reason == constants.ReasonNoResetRequired {
		return true, nil
	}
...
func (r *BMCSettingsReconciler) handleFailedState(ctx context.Context, settings *baseboardv1alpha1.BMCSettings, bmcObj *metalv1alpha1.BMC) error {
	log := ctrl.LoggerFrom(ctx)
	if utils.ShouldRetryReconciliation(settings) {
...
func (r *BMCSettingsReconciler) checkIfMaintenanceGranted(ctx context.Context, settings *baseboardv1alpha1.BMCSettings, bmcObj *metalv1alpha1.BMC, bmcClient bmc.BMC) (bool, error) {
	...
	for _, item := range settings.Spec.ServerMaintenanceRefs {
		...
		if serverMaintenance.Status.State != maintenancev1alpha1.ServerMaintenanceStateInMaintenance {
			...
			notInMaintenanceState = append(notInMaintenanceState, serverMaintenance.Name)
		}
	}
...
func (r *BMCSettingsReconciler) requestMaintenanceOnServers(ctx context.Context, settings *baseboardv1alpha1.BMCSettings, bmcObj *metalv1alpha1.BMC, bmcClient bmc.BMC) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsReconciler) getBMC(ctx context.Context, settings *baseboardv1alpha1.BMCSettings) (*metalv1alpha1.BMC, error) {
	if settings.Spec.BMCRef == nil {
...
func (r *BMCSettingsReconciler) getReferredServerMaintenances(ctx context.Context, ServerMaintenanceRefs []api.ServerMaintenanceRefItem) ([]*maintenancev1alpha1.ServerMaintenance, []error) {
	log := ctrl.LoggerFrom(ctx)
	serverMaintenances := make([]*maintenancev1alpha1.ServerMaintenance, 0, len(ServerMaintenanceRefs))
	var errs []error
...
func (r *BMCSettingsReconciler) updateBMCSettingsStatus(ctx context.Context, settings *baseboardv1alpha1.BMCSettings, state baseboardv1alpha1.BMCSettingsState, condition *metav1.Condition) error {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsReconciler) enqueueBMCSettingsByBMCVersion(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if BMCVersion.Status.State != baseboardv1alpha1.BMCVersionStateCompleted {
		return nil
	}
...
func (r *BMCSettingsReconciler) enqueueBMCSettingsBySecret(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	list := &baseboardv1alpha1.BMCSettingsList{}
	...
	for _, settings := range list.Items {
		...
		for _, v := range settings.Spec.Variables {
			...
			val, err := utils.ResolveFieldRef(&settings, utils.SubstituteVars(v.ValueFrom.FieldRef.FieldPath, partialResolved))
			if err != nil {
				continue
			}
		...
		for _, v := range settings.Spec.Variables {
			...
			if utils.SubstituteVars(ref.Name, partialResolved) == secretName && utils.SubstituteVars(ref.Namespace, partialResolved) == secretNamespace {
...
func (r *BMCSettingsReconciler) enqueueBMCSettingsByConfigMap(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	for _, settings := range list.Items {
		...
		for _, v := range settings.Spec.Variables {
			...
			if utils.SubstituteVars(ref.Name, partialResolved) == cmName && utils.SubstituteVars(ref.Namespace, partialResolved) == cmNamespace {
				requests = append(requests, ctrl.Request{
					NamespacedName: types.NamespacedName{Namespace: settings.Namespace, Name: settings.Name},
				})
				break
