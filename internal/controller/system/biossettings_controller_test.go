...
var _ = Describe("BIOSSettings Controller", func() {
	...
	BeforeEach(func(ctx SpecContext) {
		...
		server = &metalv1alpha1.Server{
			...
			Spec: metalv1alpha1.ServerSpec{
				...
				SystemURI:  "/redfish/v1/Systems/437XR1138R2",
	...
	It("should request maintenance when changing power status of server, even if bios settings update does not need it", func(ctx SpecContext) {
		...
		By("Ensuring that the Server has been claimed and powered off as requested by the claim")
		...
		Eventually(Object(biosSettings)).Should(SatisfyAll(
			HaveField("Status.State", systemv1alpha1.BIOSSettingsStateInProgress),
	...
	It("should allow retry using annotation", func(ctx SpecContext) {
		...
		Eventually(Object(biosSettings)).Should(SatisfyAll(
			HaveField("Status.State", systemv1alpha1.BIOSSettingsStateFailed),
		...
		Eventually(func(g Gomega) int {
			...
			for i := range serverMaintenanceList.Items {
				if metav1.IsControlledBy(&serverMaintenanceList.Items[i], biosSettings) {
					owned++
				}
			}
			return owned
	...
	It("should replace missing BIOSSettings ref in server", func(ctx SpecContext) {
		...
		biosSettings2 := &systemv1alpha1.BIOSSettings{
			ObjectMeta: metav1.ObjectMeta{
...
var _ = Describe("BIOSSettings Controller with BMCRef BMC", func() {
	...
	It("should request maintenance when changing power status of server, even if bios settings update does not need it", func(ctx SpecContext) {
		...
		By("Approving the maintenance")
		Eventually(Update(serverClaim, func() {
			metautils.SetLabel(serverClaim, maintenancev1alpha1.ServerMaintenanceApprovedLabelKey, "true")
		})).Should(Succeed())
			...
			SatisfyAll(
				HaveField("Status.FlowState", ContainElement(
					SatisfyAll(
						...
						HaveField("Name", "100"),
						HaveField("State", systemv1alpha1.BIOSSettingsFlowStateFailed),
					),
				)),
			),
		)
			...
			SatisfyAll(
				HaveField("Status.FlowState", ContainElement(
					SatisfyAll(
						...
						HaveField("State", systemv1alpha1.BIOSSettingsFlowStateApplied),
					),
				)),
				HaveField("Status.FlowState", ContainElement(
					SatisfyAll(
...
func ensureBiosSettingsFlowCondition(biosSettings *systemv1alpha1.BIOSSettings) {
	By("Ensuring right number of conditions are present")
	Eventually(Object(biosSettings)).Should(
	...
	Eventually(Object(biosSettings)).Should(
		HaveField("Status.Conditions", Not(ContainElement(
			HaveField("Type", constants.ConditionVersionUpdatePending),
		))),
	)
	...
	Eventually(Object(biosSettings)).Should(
		HaveField("Status.Conditions", ContainElement(
			SatisfyAll(
				HaveField("Type", constants.ConditionServerMaintenanceCreated),
				HaveField("Status", metav1.ConditionTrue),
			),
		)),
	)
	...
	Eventually(Object(biosSettings)).Should(
		HaveField("Status.Conditions", ContainElement(
			SatisfyAll(
				HaveField("Type", constants.ConditionServerMaintenanceDeleted),
				HaveField("Status", metav1.ConditionTrue),
			),
		)),
	)
...
func ensureBiosSettingsCondition(biosSettings *systemv1alpha1.BIOSSettings, RebootNeeded bool, waitForVersionUpgrade bool) {
	commonConditions := 4

	// Required flow conditions that must always be present once the flow has completed.
	// RebootObservedOff is intentionally NOT included here: it is best-effort only (the
	// transient "server briefly off" window is often too short for a reconcile to ever
	// observe it), so tests must not require it to be present.
	requiredFlowConditionTypes := []string{
		ConditionSettingsUpdateStartTime,
		ConditionSettingsServerPowerOn,
		ConditionSettingsRebootPostUpdate,
		ConditionSettingsUpdateIssued,
		ConditionSettingsVerify,
	}

	if RebootNeeded {
		requiredFlowConditionTypes = append(requiredFlowConditionTypes, ConditionSettingsRebootIssued, ConditionSettingsRebootPowerOn)
	}
	...
	By("Ensuring the required flow conditions are present")
	Eventually(Object(biosSettings)).Should(
		HaveField("Status.FlowState", WithTransform(
			func(flowStates []systemv1alpha1.BIOSSettingsFlowStatus) []metav1.Condition {
				if len(flowStates) == 0 {
					return nil
				}
				return flowStates[0].Conditions
			},
			ContainElements(requiredFlowConditionMatchers...),
		)),
	)
		...
		Eventually(Object(biosSettings)).Should(
			HaveField("Status.FlowState", ContainElement(
				HaveField("Conditions", SatisfyAll(
					...
					Not(ContainElement(HaveField("Type", ConditionSettingsRebootIssued))),
		...
		Eventually(Object(biosSettings)).Should(
			HaveField("Status.FlowState", ContainElement(
				HaveField("Conditions", SatisfyAll(
					...
					ContainElement(
						SatisfyAll(
							HaveField("Type", ConditionSettingsRebootIssued),
							HaveField("Status", metav1.ConditionTrue),
						),
					),
					ContainElement(
