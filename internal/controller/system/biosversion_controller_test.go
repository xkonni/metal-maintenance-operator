...
var _ = Describe("BIOSVersion Controller", func() {
	...
	It("should allow retry using annotation", func(ctx SpecContext) {
		...
		Eventually(Object(biosVersion)).Should(SatisfyAll(
			HaveField("Status.State", systemv1alpha1.BIOSVersionStateFailed),
...
func ensureBiosVersionConditionTransition(acc *conditionutils.Accessor, biosVersion *systemv1alpha1.BIOSVersion, server *metalv1alpha1.Server) {
	...
	Eventually(
		func(g Gomega) bool {
			g.Expect(Get(biosVersion)()).To(Succeed())
			g.Expect(acc.FindSlice(biosVersion.Status.Conditions, constants.ConditionVersionUpgradeIssued, condIssue)).To(BeTrue())
	...
	Eventually(
		func(g Gomega) bool {
			g.Expect(Get(biosVersion)()).To(Succeed())
			g.Expect(acc.FindSlice(biosVersion.Status.Conditions, constants.ConditionVersionUpgradeCompleted, condComplete)).To(BeTrue())
	...
	Eventually(func(g Gomega) bool {
		g.Expect(Get(biosVersion)()).To(Succeed())
		g.Expect(acc.FindSlice(biosVersion.Status.Conditions, constants.ConditionVersionUpgradeVerification, verificationComplete)).To(BeTrue())
