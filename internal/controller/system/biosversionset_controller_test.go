...
var _ = Describe("BIOSVersionSet Controller", func() {
	...
	It("should successfully reconcile the resource", func(ctx SpecContext) {
		By("Created resource")
		biosVersionSet := &systemv1alpha1.BIOSVersionSet{
			ObjectMeta: metav1.ObjectMeta{
		...
		biosVersion02 := &systemv1alpha1.BIOSVersion{
			ObjectMeta: metav1.ObjectMeta{
		...
		biosVersion03 := &systemv1alpha1.BIOSVersion{
			ObjectMeta: metav1.ObjectMeta{
	...
	It("should successfully reconcile the resource when BMC are deleted/created", func(ctx SpecContext) {
		...
		biosVersion01 := &systemv1alpha1.BIOSVersion{
			ObjectMeta: metav1.ObjectMeta{
	...
	It("Should successfully retry failed state child resources", func(ctx SpecContext) {
		...
		Eventually(Object(biosVersion02)).Should(SatisfyAll(
			HaveField("Status.State", Not(Equal(systemv1alpha1.BIOSVersionStateFailed))),
